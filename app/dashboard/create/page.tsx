  ...
  return (
    <button type="submit" disabled={pending} className={`w-full relative bg-brand-accent hover:bg-brand-accent/90 text-black font-oxanium font-bold text-xl py-6 rounded-lg shadow-[0_0_30px_rgba(0,240,255,0.2)] transition-all flex items-center justify-center gap-4 group overflow-hidden ${pending ? 'opacity-70 cursor-wait' : ''}`}>
      ...
      <span className="flex items-center gap-4 relative z-10">
        {pending ? <><Loader2 size={28} className="animate-spin" /> INITIALIZING UPLOAD...</> : <><Zap size={28} strokeWidth={3} /> FINALIZE DEPLOYMENT</>}
      </span>
    </button>
  );
}
              ...
              <form action={dispatch} className="space-y-8">
                <div className="space-y-6 p-6 bg-black/20 rounded-xl border border-white/5">
                  <div>
                    ...
                    <input name="title" type="text" placeholder="e.g. NEON VORTEX LIVERY" value={formData.title} onChange={handleInputChange} className={`w-full bg-black/40 border ${state.errors?.title ? 'border-red-500 animate-pulse' : 'border-white/10'} rounded-lg p-4 text-white font-oxanium font-black focus:border-brand-accent focus:outline-none uppercase tracking-widest transition-colors`} />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      {isCustomCategory && (<div className="relative animate-in fade-in slide-in-from-top-2"><input type="text" name="category" value={formData.category} onChange={handleInputChange} placeholder="TYPE CUSTOM..." className={`w-full bg-brand-accent/10 border ${state.errors?.category ? 'border-red-500' : 'border-brand-accent/50'} rounded-lg p-4 text-brand-accent font-black uppercase tracking-widest focus:outline-none`} /><Plus size={16} className="absolute right-4 top-4 text-brand-accent" /></div>)}
                ...
                <div className="space-y-6 p-6 bg-black/20 rounded-xl border border-white/5">
                  ...
                  <div>
                    ...
                    <div className="flex gap-4 items-center">
                      <input name="downloadUrl" type="text" placeholder="Link (https://...)" value={formData.downloadUrl} onChange={handleInputChange} className={`flex-grow bg-black/40 border ${state.errors?.downloadUrl ? 'border-red-500' : 'border-white/10'} rounded-lg p-4 text-white focus:border-brand-accent focus:outline-none h-[50px]`} />
                      <button type="button" onClick={() => openUploadWidget('downloadUrl')} className={`${buttonStyle} !bg-white/5 !text-white hover:!bg-white/10 !border-white/10`}><FolderOpen size={18} /> <span className="hidden sm:inline">UPLOAD</span></button>
                      <button type="button" onClick={() => openGallery('downloadUrl')} className={buttonStyle}><ImageIcon size={18} /> <span className="hidden sm:inline">LIBRARY</span></button>
                    </div>
